...
export function DownloadButton({ asset }: DownloadButtonProps) {
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    const downloadPromise = new Promise<string>((resolve, reject) => {
      ...
      xhr.onprogress = (event) => {
        if (event.lengthComputable) {
          ...
          toast.loading(`${t('download')}: ${percentComplete}%`, { id: 'download-progress' })
        } else {
          toast.loading(`${t('download')}...`, { id: 'download-progress' })
        }
      }
      ...
      xhr.onload = function () {
        if (this.status === 200) {
          ...
          toast.success(`${t('download')} ${asset.name}`, {
            description: locale === 'zh' ? '文件下载成功' : 'File downloaded successfully',
          })
    ...
    downloadPromise.catch((err: Error) => {
      toast.dismiss('download-progress')
      toast.error(locale === 'zh' ? `下载失败: ${err.message}` : `Download failed: ${err.message}`)
    })
  }
